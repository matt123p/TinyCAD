...
RegelZOG    EQU D'74';!!!   ;Regelzahl Obergrenze Standard 74  Begrenzt die langfristige automatische Modulationsstärke-Regelung die sonst der eingestellten Übersterungs
...
PrimstartRegelZahl  EQU D'73' ;!!! ;Mit dieser Regelzahl startet das Gerät beim ersten Einschalten nach dem Batterieanschließen.
...
VUMaxBegr   EQU		D'63';!!! ;schlimmstenfalls Absturz (nicht überprüft)  Legt man beide Werte auf 60, ist die Verstärkung minimal und das Gerät kann
		...
		;Ausschaltung:
		;Lichtbewertung:
LichtBewNenn   EQU  D'3' ;!!! ;Alle soviel Sekunden fließt die Beleuchtungsauswertung  in den Ausschalt-Auf/Ab-Zähler ein. Als Bruchnenner der Ausschaltgewichtung sozusagen.
...
LichtanBonus   EQU  D'4' ;!!!  ;Um soviel wird der Ausschaltzähler heraufgezählt wenn Licht an ist also die Einschaltdauer verlängert.
...
LichtKompensMalus  EQU  D'4';!!! ;hiermit kann man eigentlich alles kompensieren, egal,warum es nicht schnell genug ausschaltet. Nicht nur Licht.
...
AbsolautLo    EQU   D'70';!!!  ; Für Klassensummen  (also Lautstärkebewertende Zahlen zwischen 65 und80) wenn größer/= dieser Zahl, (Standard 68) werden 
AbsoLoBonus   EQU   D'2' ;!!!   ; soviele Bonus vergeben, wie -hier- steht. Das heißt, wenn's bei Geräuschen länger an bleiben soll,
...
AbsolautHi    EQU   D'73';!!!  ; Für Klassensummen  (also Lautstärkebewertende Zahlen zwischen 65 und80) über dieser Zahl, (Standard 72) werden
...
EinklassenMalus  EQU D'1';!!!  ; Wenn bei den fünf Messungen in der ganzen Sekunde nur eine Lautstärkeklasse vorgekommen ist(also immer dieselbe) spricht das
...
DreiklassenBonus EQU D'3' ;!!! ;Bei Drei Klassen was selten ist, aber stark für interessante Geräusche z.B.Sprache spricht, gibt es Bonusse (Standard=2)

VierklassenBonus EQU D'4' ;!!! ;Bei Vier Klassen (echt ziemlich selten, aber interessant) standardmäßig drei Bonusse.(Addiert sich nicht mit dem dreiKlassen Bonus.Entweder oder.Also keine fünf zusammen)
...
StartBonus    EQU D'60'     ;StartBonus wird nach dem Erwachen als AusschaltBonus geladen. Dafür dass es sich überhaupt eingeschaltet hat, wird eine 
...
Voxoffset   EQU   D'0' ;!!!   ;(Standard=1)  Voxoffset=0 d.h. immer unempfindlich beide empf. Stufen deaktiviert. Vermeidet Nerverei 
...
MeldeIntervall  EQU  D'27';!!!  ; Alle soviel Sekunden wird, --- falls --- das Gerät grade sendet, ein Meldeton gesendet, der Auskunft gibt, 
							...
							;Feature ab Version 27: Meldung bei jedem Werchsel.
...
Einschleichzeit EQU D'0';!!!  ;(Standard = 0h bzw.Trottelsperre 10min) Erst nach soviel  Minuten/Stunden wie hier steht macht sich das Gerät sendebereit. bis dahin wartet es relativ stromsparend ohne Aktion
...
vTonADauer		EQU  0x4F
vTonAFreq		EQU  0x50
vMyFlagsB       EQU  0x73   ;vMyFlagsB,0 set=Licht ist an clear= Licht ist aus  
							;vMyFlagsB,1 = Zwischenspeicherung von TRISB,7 während Lichtmeldetoneinspielung
...
;Licht Aender-Meldung:
			BTFSC  PORTA,4
			GOTO   Aender1
			BTFSS  vMyFlagsB,0  ;Licht ist also an
			CALL  Meldung       ;diesen Zustand melden		
			BSF    vMyFlagsB,0  ;Änderung festhalten
			GOTO   AenderFertig

Aender1		BTFSC  vMyFlagsB,0  ;Licht ist also aus
			CALL  Meldung     ;diesen Zustand melden
			BCF    vMyFlagsB,0	  ;Änderung festhalten
AenderFertig  NOP
...
Meldung 	BCF  vMyFlagsB,1  ;MultibankAdresse! My-Flag's Register2 --Zustand der Trisb,7 sichern.--
			BSF  STATUS,RP0
			BTFSC  TRISB,7
