...
Ruler::Ruler(CMultiSheetDoc *pDesign, int nDir, CRect nSize, CWnd *parent, BOOL origin)
{
	...
	if (dir == 2) {
		...
		if (hasOrigin)
		{
			button.Create(_T("+"), WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON|BS_OWNERDRAW, 
			   CRect(0,0,RULER_WIDTH,RULER_WIDTH), parent, IDM_TOOLORIGIN);
		}
...
void Ruler::OnPaint()
{
	...
	double Height;
	if (dir == 2)
		Height = GetTransform().doubleDeScale(theHeight + offset);
	else
