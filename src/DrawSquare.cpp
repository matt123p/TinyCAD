...
void CDrawSquare::Paint(CContext &dc, paint_options options)
{
	...
	else if (HasNoteText())
	{
...
#if 0	//This needs lots of work if it is to be a feature!!!  Presently, it doesn't adjust the size of the underlying data object so selection and hovering gets broken.
		//Impose a size and width rationality limit so that notes can't disappear entirely or alias strangely
		if (r.bottom - r.top < 30) {
			r.bottom = r.top + 30;	//This is the minimum height in pixels of the supported note
			sma.y = r.top;
			m_point_a.y = sma.y;
		}
		if (r.right - r.left < 30) {
			r.right = r.left + 30;	//This is the minimum height in pixels of the supported note
			smb.x = r.right;
			m_point_b.x = smb.x;
		}
#endif
		...
		//Draw the rectangle
		dc.Rectangle(r);	//Draw the rectangle

		//Form a smaller decorative rectangle and draw it
		r.left += 2;
		r.right -= 2;
		r.top += 2;
		r.bottom -= 2;
		dc.Rectangle(r);	//Draw a smaller nested rectangle as a flourish

		//Form a small margin for the note so that the text doesn't lie on top of the inner rectangle.
		r.left += 2;
		r.right -= 2;
		r.top += 2;
		r.bottom -= 2;
