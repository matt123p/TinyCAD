...
void CEditDlgTextEdit::Open(CTinyCadDoc *pDesign, CDrawingObject *pObject)
{
  ...
  if (static_cast<CDrawText*>(getObject())->dir==3)
	CheckRadioButton(TEXTBOX_LEFT,TEXTBOX_UP,TEXTBOX_LEFT);
  else
...
void CEditDlgTextEdit::OnChange()
{
  ...
  if (GetCheckedRadioButton(TEXTBOX_LEFT,TEXTBOX_UP)==TEXTBOX_LEFT)
	static_cast<CDrawText*>(getObject())->dir=3;
  else
	static_cast<CDrawText*>(getObject())->dir=0;
