...
void CTinyCadView::OnDraw(CDC* pDC)
{
  ...
  BOOL osb = 
		!pDC->IsPrinting()
	&&  m_use_offscreen_drawing
	&&  CreateBitmap( *pDC, client.Width(), client.Height() );
...

  {
	  ...
	  if (!pDC->IsPrinting())
	  {
		  ...
		  if (Start.x<0)
			dc.Rectangle(CDRect(0,Start.y,Start.x,End.y));
		  if (Start.y<0)
			dc.Rectangle(CDRect(Start.x,0,End.x,Start.y));
...
void CTinyCadView::OnSize(UINT a,int cx,int cy)
{
	...
	GetClientRect(nSize);
	if (vRuler != NULL)
		vRuler->OnNewSize(nSize);
	if (hRuler != NULL)
		hRuler->OnNewSize(nSize);
...
void CTinyCadView::SetScroll(double NewX,double NewY, bool first)
{
  ...
  // Now does this fit?
  if (px+NewX>GetCurrentDocument()->GetDetails().GetPageBoundsAsPoint().x+Xlap)
	NewX=GetCurrentDocument()->GetDetails().GetPageBoundsAsPoint().x+Xlap-px;
  if (py+NewY>GetCurrentDocument()->GetDetails().GetPageBoundsAsPoint().y+Ylap)
	NewY=GetCurrentDocument()->GetDetails().GetPageBoundsAsPoint().y+Ylap-py;
  if (NewX<-Xlap)
	NewX=-Xlap;
  if (NewY<-Ylap)
	NewY=-Ylap;
